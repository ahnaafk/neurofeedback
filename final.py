...
def main(i):
    ...
    while keep_alive == True:
        ...
        eegdf = pd.DataFrame(np.transpose(data[eeg_channels])) #creating a dataframe of the eeg data to extract eeg values later 
        ...
        eeg1= eegdf.iloc[:, 0].values #I am using OpenBCI Ganglion board, so I only have four channels.
        eeg2=eegdf.iloc[:, 1].values  #If you have a different board, you should be able to copy paste
        eeg3=eegdf.iloc[:, 2].values  #these commands for more channels. 
        eeg4=eegdf.iloc[:, 3].values
        timex=timedf.iloc[:, 0].values #timestamps
